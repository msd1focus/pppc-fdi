...
public class AddProdukItemUserBackingBean implements Serializable {
    ...
    private final String SELECTED_ITEM_ITERATOR =
        "AppUserProdukItemView1Iterator";
    private final String ALL_ITEM_ITERATOR = "AllUserProdukItemShuttleView1Iterator";
    
    public AddProdukItemUserBackingBean() {
        super();
    }
    ...
    public String processShuttle() {
        int actualSelectedItemSize = selectedItems == null ? 0 : selectedItems.size();
        ...
        //Removing all item rows
        for (Row r : iterArea.getAllRowsInRange()) {
            r.remove();
        }

        if (actualSelectedItemSize > 0) {
