...
public class BatchPromoBarangBean {
    private RichPanelFormLayout frmStatus;

    public BatchPromoBarangBean() {
    }

    public void promoBarangBatch(ActionEvent actionEvent) {        
        Connection conn = null;
        //String getPromoBarangSql = "{CALL FCS_BATCH_REALISASI_XXX()}";
        updateBatchLog("BRPBR", "INPROCESS");
        ...
        try {
            ...
            PreparedStatement statement =
                conn.prepareCall(getPromoBarangSql);
            statement.executeUpdate();
            statement.close();
            conn.close();
            */
            updateBatchLog("BRPBR", "FINISHED");
            AdfFacesContext.getCurrentInstance().addPartialTarget(frmStatus);
        } catch (Exception exc) {
            updateBatchLog("BRPBR", "ERROR");
            AdfFacesContext.getCurrentInstance().addPartialTarget(frmStatus);
            JSFUtils.addFacesErrorMessage(exc.getMessage());
        }
    }
    
    public void promoBarangReset(ActionEvent actionEvent) {        
        updateBatchLog("BRPBR",  "-");
    }
    ...
    public void updateBatchLog(String batchStatus, String batchName) {
        ...
        masterDataAM.getBatchPromoBarangLogView1().executeQuery();
    }
